...
class AntiRaid {
  ...
  async checkJoinRaid(member) {
    ...
    if (filtered.length >= joinThreshold) {
      console.log(`⚠️ [RAID] ${member.guild.name} - ${filtered.length} kullanıcı ${joinInterval/1000}s içinde katıldı!`);
      ...
      db.findOneAndUpdate(member.guild.id, {
        $inc: { 'stats.totalRaidsBlocked': 1 },
        $set: { 'stats.lastRaidAt': new Date().toISOString() },
      });
    }
  }
  ...
  async checkSpam(message) {
    ...
    if (filtered.length >= spamThreshold) {
      ...
      await this.log(message.guild, guildData, {
        ...
        reason: `${filtered.length} mesaj ${spamInterval/1000}s içinde`,
        action: guildData.antiRaid.action,
        timestamp: new Date().toISOString(),
      });
      spamTracker.delete(key);
      db.findOneAndUpdate(message.guild.id, { $inc: { 'stats.totalSpamBlocked': 1 } });
    }
  }
  ...
  async checkMentionSpam(message) {
    ...
    if (filtered.length >= mentionThreshold) {
      ...
      await this.punish(message.member, message.guild, 'Mention spam', guildData.antiRaid.action, guildData);
      await this.log(message.guild, guildData, {
        type: 'mention_spam',
        userId: message.author.id,
        username: message.author.tag,
        reason: `${filtered.length} mention ${mentionInterval/1000}s içinde`,
        action: guildData.antiRaid.action,
        timestamp: new Date().toISOString(),
      });
  ...
  async checkChannelSpam(guild, userId, guildData) {
    ...
    if (filtered.length >= channelThreshold) {
      ...
      if (!member) return;
      await this.punish(member, guild, 'Kanal spam', 'ban', guildData);
      await this.log(guild, guildData, {
        type: 'channel_spam',
        userId,
        username: member.user.tag,
        reason: `${filtered.length} kanal işlemi ${channelInterval/1000}s içinde`,
        action: 'ban',
        timestamp: new Date().toISOString(),
      });
  ...
  async punish(member, guild, reason, action, guildData) {
    ...
    // Ban koruması
    const protected_ = (process.env.PROTECTED_USERS || '').split(',').map(s => s.trim()).filter(Boolean);
    if (protected_.includes(member.id)) return;

    try {
      switch (action) {
        case 'ban':
          await member.ban({ reason: `[AntiRaid] ${reason}` });
          db.findOneAndUpdate(guild.id, { $inc: { 'stats.totalBans': 1 } });
          break;
        case 'kick':
          await member.kick(`[AntiRaid] ${reason}`);
          db.findOneAndUpdate(guild.id, { $inc: { 'stats.totalKicks': 1 } });
          break;
        case 'mute': {
          const muteRole = guild.roles.cache.find(r => r.name.toLowerCase() === 'muted');
          if (muteRole) {
            await member.roles.add(muteRole, `[AntiRaid] ${reason}`);
            db.findOneAndUpdate(guild.id, { $inc: { 'stats.totalMutes': 1 } });
          } else {
            await member.kick(`[AntiRaid] ${reason}`);
            db.findOneAndUpdate(guild.id, { $inc: { 'stats.totalKicks': 1 } });
          }
  ...
  async disableLockdown(guild, guildData) {
    try {
      db.findOneAndUpdate(guild.id, { 'antiRaid.lockdown': false });
      const everyoneRole = guild.roles.everyone;
      await everyoneRole.setPermissions(
  ...
  async log(guild, guildData, logEntry) {
    db.findOneAndUpdate(guild.id, {
      $push: { logs: { $each: [logEntry], $slice: -100 } },
    });
    ...
    const typeEmoji = {
      raid_detected: '🚨', spam: '🗑️',
      mention_spam: '📢', channel_spam: '📁', new_account: '👶',
    };
